...
begin
  ...
  while not tblBirds.Eof do
  begin
    if (pos('macaw', lowercase(tblBirds['Classification'])) > 0) AND
      (pos('yellow', lowercase(tblBirds['Colour'])) > 0) then
    begin
      redOutput.Lines.Add(tblBirds['RefNum'] + #9 + tblBirds['Classification']
          + DateToStr(tblBirds['HatchDate']) + #9 + 'R ' + FloatToStr
          (tblBirds['Price']) + #9 + intToStr(tblBirds['StoreID']));
    end;
    tblBirds.Next;
  end;
end;
